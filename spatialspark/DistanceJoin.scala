...
object DistanceJoin {
  ...
  def main(args: Array[String]) {
    ...
    val conf = new SparkConf().setAppName("SpatialSpark Distance Join")
    ...
    def query(leftInput: String, rightInput: String, leftGeomType: String, rightGeomType: String) {
      ...
      println("Total Read Time: " + read_time + " sec")
      ...
      // Embed timers inside SpatialSaprk PartitionedJoin to get partitioning and indexing time. Subtract the partitioning and indexing time from join time below to get purely join time
      t0 = System.nanoTime()
      matchedPairs = PartitionedSpatialJoin(sc, leftGeometryById, rightGeometryById, SpatialOperator.WithinD, 0.000045027, partConf)
